...
const OPPORTUNITY_SYSTEM = [
  ...
  'CONCEPT SEED (optional): if a conceptSeed is provided, the founder has ALREADY agreed to make THIS specific reel',
  'and filmed for it. Your job is to REALIZE that exact concept using these clips — do NOT invent a different angle.',
  'Use its communicationJob/narrativeArc as the intent and its roleHints as GUIDANCE for which clip serves which beat',
  '(a candidate + a usable window). You still choose the exact in/out, the strongest subset, ordering, and which weak',
  'clips to exclude. The seed carries NO claims — hookText/ctaText must still be licensed by the strategy only.',
...
export class AnthropicReelOpportunityModel implements IReelOpportunityModelPort {
  ...
  async recommend(input: ReelOpportunityInput): Promise<ReelOpportunityDraft> {
    ...
    const user = [
      ...
      input.conceptSeed ? `CONCEPT SEED (REALIZE this exact concept, do not drift): ${JSON.stringify({ communicationJob: input.conceptSeed.communicationJob, narrativeArc: input.conceptSeed.narrativeArc, editingEnergy: input.conceptSeed.editingEnergy, targetDurationMs: input.conceptSeed.targetDurationMs, ctaDirection: input.conceptSeed.ctaDirection ?? null, roleHints: input.conceptSeed.roleHints ?? [] })}` : '',
